...
__device__ uint32_t get_idx_strides(size_t gid, CudaVec shape,CudaVec strides){
    // map gid to multi-indices, based on strides and offset 
    // gid = sum_k ik x shape[k], idx = sum_k ik x stride[k]

    
    uint32_t dim = shape.size; // dim of array a
    uint32_t indices_k = 0;
    uint32_t idx = 0; 

    for (size_t k = 0; k < dim; ++k) {
        uint32_t prod_idx = 1;
        for (size_t s = k + 1; s < dim; ++s){
        prod_idx *= shape.data[s]; 
        }
      indices_k = gid/prod_idx;
      gid -=   indices_k * prod_idx;
      idx += indices_k * strides.data[k];  // get the current idx
    }
...
__global__ void CompactKernel(const scalar_t* a, scalar_t* out, size_t size, CudaVec shape,
  ...
  /// BEGIN YOUR SOLUTION
  uint32_t idx = get_idx_strides(gid, shape, strides);
  // map it to out[gid]
  if (gid < size){
...
__global__ void ScalarPowerKernel(const scalar_t* a, scalar_t val, scalar_t* out, size_t size) {
  size_t gid = blockIdx.x * blockDim.x + threadIdx.x;
  if (gid < size) out[gid] = pow(a[gid] + 0.0000001f, val);
}
...
__global__ void MatmulKernel(const float *A, const float *B, float *C, 
   ...
   /* shared memory with As Bs TILE X TILE, works if TILE number equals to block TPB
   padd zero if sizes not divisible
   parallel at (M/TILE x N/TILE) x (N/TILE  x P/TILE) level
   ...
   */
    ...
    for (int k = 0; k < (TILE_DIM + N - 1)/TILE_DIM; k++) {
        ...
        if (k*TILE_DIM + threadIdx.x < N && Row < M)	
            As[threadIdx.y][threadIdx.x] = A[Row*N + k*TILE_DIM + threadIdx.x];
	    else													
            As[threadIdx.y][threadIdx.x] = 0.0;

	if (k*TILE_DIM + threadIdx.y < N && Col < P)	
        Bs[threadIdx.y][threadIdx.x] = B[(k*TILE_DIM + threadIdx.y)*P + Col];
	else													
    ...
    if (Row < M && Col < P) 
        C[((blockIdx.y * blockDim.y + threadIdx.y)*P)+(blockIdx.x*blockDim.x)+threadIdx.x]=CValue;
}
...
__global__ void NaiveMatmulKernel(const float *A, const float *B, float *C, 
                          int M, int N, int P) {
                          /**
   /* parallel to M x P level, inner product compute element C(m, p)
   no handling of arbitrary sizes not divisible
   */
...
void Matmul(const CudaArray& a, const CudaArray& b, CudaArray* out, uint32_t M, uint32_t N,
  ...
  /// BEGIN YOUR SOLUTION
  size_t grid_y = (M + THREAD_PER_BLOCK - 1) / THREAD_PER_BLOCK;
  size_t grid_x = (P + THREAD_PER_BLOCK - 1) / THREAD_PER_BLOCK;
  dim3 grids = dim3(grid_x, grid_y);
  dim3 blocks = dim3(THREAD_PER_BLOCK, THREAD_PER_BLOCK);
  MatmulKernel<<<grids, blocks>>>(a.ptr, b.ptr, out->ptr, M, N, P);
  /// END YOUR SOLUTION
}
...
if (gid<size){
  for (size_t j = 0; j < reduce_size; j++){
      if (j == 0){
          out[gid] = a[gid*reduce_size + j];
      }
      else{
          out[gid] = max(a[gid*reduce_size + j], out[gid]); 
      }
   }
   }
...
__global__ void ReduceSumKernel(const scalar_t* a, scalar_t* out, size_t reduce_size, size_t size) {
  ...
  for (size_t j = 0; j < reduce_size; j++){
      if (j == 0){
          ...
          out[gid] = a[gid*reduce_size + j] + out[gid]; 
      }
   }
   }
