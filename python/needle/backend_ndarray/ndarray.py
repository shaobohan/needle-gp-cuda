...
class NDArray:
    ...
    def permute(self, new_axes):
        ...
        _strides = tuple(self.strides[i] for i in new_axes)
        # _strides = self.compact_strides(_shape) # first dim changed
        # print(_strides)
        # print(_strides2)
        return self.as_strided(_shape, _strides)
        ### END YOUR SOLUTION
    ...
    def broadcast_to(self, new_shape):
        ...
        for idx in range(len(self.shape)):
            if self.shape[idx] != 1: 
                if new_shape[idx] != self.shape[idx]: 
                    raise AsserionError('new_shape elements must be the same as the original shape, except for dimensions in the self where the size = 1') 
            
        _strides_lst = list()
        for i in range(len(new_shape)):
            if new_shape[i] == self.shape[i]: 
                _strides_lst.append(self.strides[i])
            else:
                _strides_lst.append(0)
        
        _strides = tuple(_strides_lst)
    ...
    def __getitem__(self, idxs):
        ...
        for i in range(len(idxs)):
            ...
            if stop > self.shape[i]: 
                stop = slef.shape[i]
            offset_v += start * _strides[i]
            new_shape_lst.append(int(np.ceil((stop-start)/step)))
            new_strides_lst.append(_strides[i] * step)
            
        new_shape = tuple(new_shape_lst)
        new_strides = tuple(new_strides_lst)
        return NDArray.make(new_shape, new_strides, device=self.device, handle=self._handle, offset=offset_v)
        ### END YOUR SOLUTION
